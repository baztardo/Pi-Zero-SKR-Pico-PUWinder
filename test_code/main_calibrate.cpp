// Hall Sensor Calibration Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Calibrate_v1.0\n");
    }
        ...
        // CALIBRATION: Calculate RPM with different pulse rates
        ...
        if (elapsed_us > 100000) {  // Calculate every 100ms
            ...
            if (elapsed_sec > 0 && hall_delta > 0) {
                ...
                // Try different pulse rates per revolution
                float rpm_3 = (pulses_per_sec / 3.0f) * 60.0f;   // 3 pulses/rev
                float rpm_6 = (pulses_per_sec / 6.0f) * 60.0f;   // 6 pulses/rev
                float rpm_12 = (pulses_per_sec / 12.0f) * 60.0f; // 12 pulses/rev
                float rpm_60 = (pulses_per_sec / 60.0f) * 60.0f; // 60 pulses/rev
                
                char response[128];
                snprintf(response, sizeof(response), "RPM_3:%.1f RPM_6:%.1f RPM_12:%.1f RPM_60:%.1f\n", 
                        rpm_3, rpm_6, rpm_12, rpm_60);
                uart_puts(PI_UART_ID, response);
                
                printf("Hall RPM - 3:%.1f 6:%.1f 12:%.1f 60:%.1f (%.1f pulses/sec)\n", 
                       rpm_3, rpm_6, rpm_12, rpm_60, pulses_per_sec);
            } else {
                uart_puts(PI_UART_ID, "RPM_3:0.0 RPM_6:0.0 RPM_12:0.0 RPM_60:0.0\n");
            }
...
int main() {
    ...
    printf("Hall Sensor Calibration v1.0\n");
