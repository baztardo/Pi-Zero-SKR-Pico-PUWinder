// Simple Z-Index Encoder RPM Firmware (No Filtering)
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Simple_Encoder_v1.0\n");
    }
        ...
        // SIMPLE: Z-INDEX RPM with NO filtering
        uint32_t current_time = time_us_32();
        ...
        if (elapsed_sec > 0.1f && z_delta > 0) {  // At least 100ms and some Z-index pulses
            // SIMPLE: Direct calculation, no filtering
            ...
            printf("Z-index RPM: %.1f (%.1f rev/sec, %u delta, no filtering)\n", 
...
int main() {
    ...
    printf("Simple Z-Index Encoder RPM v1.0\n");
    ...
    printf("Z-index: 1 pulse per revolution (NO filtering)\n");
