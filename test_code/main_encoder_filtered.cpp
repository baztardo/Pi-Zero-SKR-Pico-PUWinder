// Filtered Z-Index Encoder RPM Firmware
...
// Z-index filtering
uint32_t z_debounce_count = 0;
uint32_t z_debounce_threshold = 3;  // Require 3 consecutive readings
uint32_t z_stable_count = 0;
bool z_stable_state = false;
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Filtered_Encoder_v1.0\n");
    }
        ...
        // FILTERED: Z-INDEX RPM with noise filtering
        uint32_t current_time = time_us_32();
        ...
        if (elapsed_sec > 1.0f && z_delta > 0) {  // At least 1 second and some Z-index pulses
            // FILTERED: Use filtered Z-index count
            float revolutions_per_sec = z_delta / elapsed_sec;
            ...
            printf("Z-index RPM: %.1f (%.1f rev/sec, %u delta, filtered)\n", 
...
int main() {
    ...
    printf("Filtered Z-Index Encoder RPM v1.0\n");
    ...
    printf("Z-index: 1 pulse per revolution (with noise filtering)\n");
    ...
    while (true) {
        ...
        // FILTERED: Check Encoder Z-index with debouncing
        bool current_z_state = gpio_get(ENCODER_Z_PIN);
        
        if (current_z_state == z_stable_state) {
            z_debounce_count = 0;
        } else {
            z_debounce_count++;
            if (z_debounce_count >= z_debounce_threshold) {
                z_stable_state = current_z_state;
                z_debounce_count = 0;
                
                if (z_stable_state && !last_z_state) {  // Rising edge
                    z_count++;
                    printf("Z-index detected! Count: %u (filtered)\n", z_count);
                }
                last_z_state = z_stable_state;
            }
        }
