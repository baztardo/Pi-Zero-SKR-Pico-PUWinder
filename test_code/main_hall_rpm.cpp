// Hall Sensor RPM Calculation Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Hall_RPM_v1.0\n");
    }
        ...
        if (elapsed_sec > 0) {
            float pulses_per_sec = hall_count / elapsed_sec;
            // Assuming 3 pulses per revolution (typical BLDC)
            float rpm = (pulses_per_sec / 3.0f) * 60.0f;
            
            char response[64];
            snprintf(response, sizeof(response), "HALL_RPM:%.1f\n", rpm);
            uart_puts(PI_UART_ID, response);
            
            printf("Hall RPM: %.1f (%.1f pulses/sec)\n", rpm, pulses_per_sec);
        } else {
...
int main() {
    ...
    printf("Hall Sensor RPM Calculation v1.0\n");
