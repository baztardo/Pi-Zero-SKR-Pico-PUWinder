// Fixed A/B/Z Encoder Test Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Encoder_Fixed_v1.0\n");
    }
        ...
        // FIXED: Encoder RPM calculation (1440 PPR)
        uint32_t current_time = time_us_32();
        ...
        if (elapsed_us > 500000) {  // Calculate every 500ms
            ...
            if (elapsed_sec > 0 && abs(pos_delta) > 0) {
                ...
                printf("Encoder RPM: %.1f (%.1f pulses/sec, %d delta, %.1f sec)\n", 
                       rpm, pulses_per_sec, pos_delta, elapsed_sec);
            } else {
                uart_puts(PI_UART_ID, "ENCODER_RPM:0.0\n");
            }
...
int main() {
    ...
    printf("Fixed A/B/Z Encoder Test v1.0\n");
