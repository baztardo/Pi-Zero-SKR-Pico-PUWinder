// Polling-based RPM Calculation Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Polling_v1.0\n");
    }
        ...
        // POLLING: Calculate RPM using simple polling method
        ...
        if (elapsed_us > 1000000) {  // Calculate every 1 second
            ...
            if (elapsed_sec > 0 && hall_delta > 0) {
                ...
                // POLLING: 19 pulses per revolution
...
int main() {
    ...
    printf("Polling RPM Calculation v1.0\n");
    printf("BLDC: PWM:%d, Enable:%d, Hall:%d\n", BLDC_PWM_PIN, BLDC_ENABLE_PIN, BLDC_HALL_PIN);
    printf("Hall sensor: 19 pulses per revolution (polling method)\n");
