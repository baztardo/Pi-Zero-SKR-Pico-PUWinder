...
// Encoder pins (adjust for your wiring)
#define ENCODER_A_PIN 2
#define ENCODER_B_PIN 3
#define ENCODER_Z_PIN 4
#define ENCODER_PULSE_PIN 22  // SC speed pulse from controller
...
void encoder_z_isr(uint gpio, uint32_t events) {
    if (gpio == ENCODER_Z_PIN) {
        z_count++;
        last_z_time = time_us_32();
        printf("Z-index detected! Total: %u\n", z_count);
    }
}
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Encoder_ABZ_Test_v1.0\n");
    }
        ...
        char response[128];
        snprintf(response, sizeof(response), "A:%u B:%u Z:%u PULSE:%u\n", 
                a_count, b_count, z_count, pulse_count);
        uart_puts(PI_UART_ID, response);
        
        printf("Encoder counts - A: %u, B: %u, Z: %u, Pulse: %u\n", 
               a_count, b_count, z_count, pulse_count);
    }
    else if (strcmp(cmd, "GET_RPM") == 0) {
        // Calculate RPM from A/B encoder (1440 PPR)
        uint32_t current_time = time_us_32();
        uint32_t a_delta = a_count;
        uint32_t b_delta = b_count;
        
        // Reset counters for next measurement
        a_count = 0;
        b_count = 0;
        
        uint32_t total_pulses = a_delta + b_delta;
        float rpm = (total_pulses / 1440.0f) * 60.0f;  // Convert to RPM
        
        char response[64];
        snprintf(response, sizeof(response), "RPM:%.1f\n", rpm);
        uart_puts(PI_UART_ID, response);
        
        printf("Calculated RPM: %.1f (from %u pulses)\n", rpm, total_pulses);
    }
...
int main() {
    ...
    printf("A/B/Z Encoder Test v1.0\n");
    printf("Encoder: 360 PPR, 1440 total pulses/rev\n");
    ...
    printf("Pins: A:%d B:%d Z:%d Pulse:%d\n", ENCODER_A_PIN, ENCODER_B_PIN, ENCODER_Z_PIN, ENCODER_PULSE_PIN);
    printf("Commands: SET_BLDC_RPM, STOP_BLDC, GET_ENCODER, GET_RPM, RESET_ENCODER\n");
