...
// Purpose: Test if the basic UART and BLDC motor functionality works
// =============================================================================
...
void process_command(const char* cmd) {
    ...
    else if (strcmp(cmd, "TEST_HALL") == 0) {
        // Test Hall sensor
        if (spindle_controller) {
            uint32_t pulses = spindle_controller->get_pulse_count();
            float rpm = spindle_controller->get_rpm();
            char response[64];
            snprintf(response, sizeof(response), "HALL_PULSES:%u RPM:%.1f\n", pulses, rpm);
            uart_puts(PI_UART_ID, response);
        } else {
            uart_puts(PI_UART_ID, "HALL_ERROR\n");
        }
    }
    else {
...
int main() {
    ...
    printf("Commands: PING, VERSION, SET_SPINDLE_RPM <rpm>, GET_SPINDLE_RPM, STOP_SPINDLE, TEST_HALL\n");
