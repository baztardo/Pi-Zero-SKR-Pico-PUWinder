// Light Z-Index Encoder RPM Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Light_Encoder_v1.0\n");
    }
        ...
        // LIGHT: Z-INDEX RPM with minimal filtering
        uint32_t current_time = time_us_32();
        ...
        if (elapsed_sec > 0.2f && z_delta > 0) {  // At least 200ms and some Z-index pulses
            // LIGHT: Simple calculation with minimal filtering
            ...
            printf("Z-index RPM: %.1f (%.1f rev/sec, %u delta, light filtering)\n", 
...
int main() {
    ...
    printf("Light Z-Index Encoder RPM v1.0\n");
    ...
    printf("Z-index: 1 pulse per revolution (light filtering)\n");
