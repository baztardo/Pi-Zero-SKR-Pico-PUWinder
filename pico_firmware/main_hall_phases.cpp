// Hall Sensor Phase-based RPM Calculation Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Hall_Phases_v1.0\n");
    }
        ...
        // HALL PHASES: Calculate RPM using Hall sensor phase changes
        ...
        if (elapsed_us > 1000000) {  // Calculate every 1 second
            ...
            if (elapsed_sec > 0 && hall_delta > 0) {
                ...
                // HALL PHASES: 6 phase changes per revolution
                // But we need to account for the 3x factor we observed
                float rpm = (phase_changes_per_sec / 6.0f) * 60.0f;
...
int main() {
    ...
    printf("Hall Sensor Phase-based RPM v1.0\n");
    printf("BLDC: PWM:%d, Enable:%d, Hall:%d\n", BLDC_PWM_PIN, BLDC_ENABLE_PIN, BLDC_HALL_PIN);
    printf("Hall sensor: 6 phase changes per revolution\n");
