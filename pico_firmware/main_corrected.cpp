// Corrected RPM Calculation Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Corrected_v1.0\n");
    }
        ...
        // CORRECTED: Calculate RPM with 19 pulses per revolution (from scope data)
        ...
        if (elapsed_us > 100000) {  // Calculate every 100ms
            ...
            if (elapsed_sec > 0 && hall_delta > 0) {
                ...
                // CORRECTED: 19 pulses per revolution (from scope: 150Hz at 479RPM)
...
int main() {
    ...
    printf("Corrected RPM Calculation v1.0\n");
    printf("BLDC: PWM:%d, Enable:%d, Hall:%d\n", BLDC_PWM_PIN, BLDC_ENABLE_PIN, BLDC_HALL_PIN);
    printf("Hall sensor: 19 pulses per revolution (from scope data)\n");
