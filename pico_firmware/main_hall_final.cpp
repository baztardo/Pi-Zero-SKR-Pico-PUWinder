// Final Hall Sensor RPM Calculation Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Hall_Final_v1.0\n");
    }
        ...
        // FINAL: Calculate RPM with 36 phase changes per revolution (6 × 6)
        ...
        if (elapsed_us > 1000000) {  // Calculate every 1 second
            ...
            if (elapsed_sec > 0 && hall_delta > 0) {
                ...
                // FINAL: 36 phase changes per revolution (6 × 6)
...
int main() {
    ...
    printf("Final Hall Sensor RPM v1.0\n");
    printf("BLDC: PWM:%d, Enable:%d, Hall:%d\n", BLDC_PWM_PIN, BLDC_ENABLE_PIN, BLDC_HALL_PIN);
    printf("Hall sensor: 36 phase changes per revolution (6 × 6)\n");
