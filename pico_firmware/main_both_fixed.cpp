// Both Hall and Encoder RPM Fixed Firmware
...
void process_command(const char* cmd) {
    ...
    if (strcmp(cmd, "PING") == 0) {
        ...
        uart_puts(PI_UART_ID, "Both_Fixed_v1.0\n");
    }
        ...
        // FIXED: Hall sensor RPM calculation (36 triggers per revolution)
        ...
        if (elapsed_us > 1000000) {  // Calculate every 1 second
            ...
            if (elapsed_sec > 0 && hall_delta > 0) {
                ...
                float rpm = (phase_changes_per_sec / 36.0f) * 60.0f;  // FIXED: 36 instead of 18
        ...
        // FIXED: Encoder RPM calculation with better debugging
        uint32_t current_time = time_us_32();
        
        if (last_encoder_time == 0) {
            last_encoder_time = current_time;
            last_encoder_pos = encoder_position;
            printf("Encoder RPM: Initializing...\n");
            uart_puts(PI_UART_ID, "ENCODER_RPM:0.0\n");
            return;
        }
        
        uint32_t elapsed_us = current_time - last_encoder_time;
        if (elapsed_us > 200000) {  // Calculate every 200ms (shorter window)
            int32_t pos_delta = encoder_position - last_encoder_pos;
            float elapsed_sec = elapsed_us / 1000000.0f;
            
            printf("Encoder RPM: pos_delta=%d, elapsed=%.3f, pos=%d->%d\n", 
                   pos_delta, elapsed_sec, last_encoder_pos, encoder_position);
            
            if (elapsed_sec > 0 && abs(pos_delta) > 0) {
                float pulses_per_sec = abs(pos_delta) / elapsed_sec;
                float rpm = (pulses_per_sec / 1440.0f) * 60.0f;
                
                char response[64];
                snprintf(response, sizeof(response), "ENCODER_RPM:%.1f\n", rpm);
                uart_puts(PI_UART_ID, response);
                
                printf("Encoder RPM: %.1f (%.1f pulses/sec, %d delta)\n", 
                       rpm, pulses_per_sec, pos_delta);
            } else {
                uart_puts(PI_UART_ID, "ENCODER_RPM:0.0\n");
            }
...
int main() {
    ...
    printf("Both Hall and Encoder RPM Fixed v1.0\n");
    ...
    printf("Encoder: 1440 PPR (360 PPR × 4 quadrature)\n");
