...
COPY THIS CONTENT TO YOUR CM4 AS motion_planner.py
"""
...
class PicoMotionPlanner:
        ...
        while time.time() - start_time < timeout:
            if self.serial.in_waiting:
                ...
                if "RESPONSE:" in response:
                    # Continue reading for a bit more to get the complete response
                    time.sleep(0.1)
                    if self.serial.in_waiting:
                        extra_data = self.serial.read(self.serial.in_waiting).decode('utf-8', errors='ignore')
                        response += extra_data
                    break
            time.sleep(0.01)

        # Debug: print what we received
        print(f"DEBUG: Command '{command}' -> Response: {repr(response)}")
    ...
    def home_machine(self):
        ...
        if "RESPONSE:" in response and ("HOMING" in response or "OK" in response):
    ...
    def set_spindle_speed(self, rpm: float):
        ...
        if "RESPONSE:" in response and "OK" in response:
...
def main():
    ...
    # Example coil specification - MODIFY THESE VALUES FOR YOUR COIL
    coil = CoilSpec(
